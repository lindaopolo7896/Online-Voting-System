...
const STATUS_STYLES = {
  ...
  withdrawn: "bg-orange-500/10 border-orange-500/20 text-orange-500",
        ...
        return (
          <span
            className={`inline-flex min-w-27.5 justify-center rounded-md border px-3 py-1 text-xs font-semibold uppercase ${
              STATUS_STYLES[status] ?? "bg-muted/10 border-muted/20 text-muted"
            }`}
          >
            {status}
          </span>
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded border border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5"
          >
            <Pencil size={15} />
          </button>
        </div>
